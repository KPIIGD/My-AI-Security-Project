"""NER Owner wrapper — fine-tuned Korean PII NER v3 runtime.

이 모듈은 NER Owner (김민우) 가 작성한 klue/roberta-large fine-tuned 모델의 raw
backend wrapper. HuggingFace `transformers` runtime 위에서 동작하며, BIO →
contiguous char span 변환 + heuristic postprocess + threshold filtering 을 수행한다.

v0.2 통합 경로:
  pii_guardrail.ner.finetuned_wrapper.FinetunedNERDetector (adapter)
    ↓ importlib.import_module("pii_guardrail.ner.owner_wrapper")
  pii_guardrail.ner.owner_wrapper.FinetunedKoreanNERDetector (이 모듈)
    ↓ from_pretrained("vmaca123/korean-pii-ner-v3")
  HuggingFace Hub (model.safetensors + tokenizer + calibration.json)

출력 형식 (PIISpan 변환 전 dict):
  [{
...
PIISpan contract 보장:
  - start, end 는 raw text char offset
  - text == raw_text[start:end] 100% 성립
  - entity_type 은 v0.2 EntityType enum value 와 매칭
  - score [0, 1]
  - reason_codes ≥ 1

NER scope: NAME → PERSON_NAME, ADDRESS → ADDRESS_FULL, ORG → ORGANIZATION
(SCHOOL/HOSPITAL/ADDRESS_UNIT 등 세분 entity 는 후속 dict/resolver 책임)

Optional dependency:
  `transformers`, `torch` 가 설치되어야 함. v0.2 패키지의 `[ner]` extra 로 설치 가능.

Conjunctive 패턴 처리:
  v3 학습 데이터에 conjunctive composite augment 포함되어 모델 자체가 대부분 처리.
  Heuristic split (`_split_address_name_conjunction`) 은 안전망으로 유지 — 모델이
  ADDRESS span 에 NAME 을 빨아들이는 edge case 에만 트리거.

v3 모델 평가 결과:
  - Internal test macro-F1 0.878 (3,401 samples)
  - KLUE-NER validation macro-F1 0.766 (5,000 samples)

자세한 설계 / 학습 결과는 `docs/14_NER_DESIGN_v1.md` 참조.
"""
