...
Date: 2026-05-22  
Status: Draft for test expansion and fix direction
...
현재 기준은 `reports/release_gate_v0_2.json`의 5,000개 synthetic release gate 결과다.
...
| 오류 유형 | 건수 | 해석 |
|---|---:|---|
| 오탐 | 1,450 | 개인정보가 아닌데 잡은 경우가 가장 많다. 정밀도 개선의 핵심이다. |
| 미탐 | 297 | 개인정보인데 못 잡은 경우다. 일부 주소 일부, 법인등록번호, 사람 이름에서 발생한다. |
| 유형 혼동 | 285 | 대부분 주소 일부와 주소 전체 구분 문제, 법인등록번호와 주민등록번호 혼동 문제다. |
| 과다 탐지 | 12 | 사람 이름 span이 필요한 범위보다 넓게 잡힌 경우다. |

개인정보 유형별 주요 오류는 다음과 같다.

| 유형 | 주요 문제 | 건수 |
|---|---|---:|
| PERSON_NAME, 사람 이름 | 오탐 | 825 |
| PHONE_MOBILE, 휴대전화 번호 | 오탐 | 375 |
| ADDRESS_FULL, 주소 전체 | 유형 혼동 | 280 |
| ADDRESS_UNIT, 주소 일부 | 미탐 | 267 |
| ADDRESS_FULL, 주소 전체 | 오탐 | 125 |
| EMAIL, 전자우편 주소 | 오탐 | 125 |
| PERSON_NAME, 사람 이름 | 미탐 | 25 |
| PERSON_NAME, 사람 이름 | 과다 탐지 | 12 |
| CORPORATE_REG_NO, 법인등록번호 | 미탐 | 5 |
| RRN, 주민등록번호 | 유형 혼동 | 5 |
...
| 우선순위 | 작업 | 이유 |
|---:|---|---|
| 1 | 사람 이름 오탐 감소 | 전체 오류 중 가장 크고 hard negative 품질에 직접 영향을 준다. |
| 2 | 주소 일부와 주소 전체 분류 기준 보정 | 유형 혼동과 미탐이 함께 발생해 평가 점수 손실이 크다. |
| 3 | 휴대전화 번호 예시 문맥 감점 | 재현율은 유지하면서 정밀도를 올릴 수 있는 영역이다. |
| 4 | 전자우편 예시 문맥 감점 | 문서 예시와 실제 전자우편을 구분해야 한다. |
| 5 | 법인등록번호와 주민등록번호 우선순위 보정 | 건수는 적지만 고위험 구조형 개인정보라 정확성이 중요하다. |
| 6 | latency 별도 계측 | real NER 경로와 deterministic-only 경로를 나눠 release gate 기준을 명확히 한다. |
...
| 지표 | 현재 | 1차 목표 |
|---|---:|---:|
| 전체 정밀도 | 0.7350 | 0.8200 이상 |
| 전체 재현율 | 0.9422 | 0.9300 이상 유지 |
| 고위험 개인정보 재현율 | 0.9921 | 0.9900 이상 유지 |
| hard negative 오탐 | 825 | 300 이하 |
| PERSON_NAME 오탐 | 825 | 300 이하 |
| ADDRESS_UNIT 재현율 | 0.0000 | 0.8000 이상 |
...
| 전체 F1 | 0.8800 이상 |
| 전체 정밀도 | 0.8800 이상 |
| hard negative 오탐 | 100 이하 |
| 주소 전체와 주소 일부 유형 혼동 | 현재 대비 70% 이상 감소 |
...
| bucket | 건수 | 목적 |
|---|---:|---|
| `hard_negative_phone` | 400 | 예시, 샘플, 더미, placeholder, fixture, 문서용, 목업, 교육 자료, 검증용, 스토리북, seed, synthetic, 데모, 템플릿, 가이드, 샌드박스 문맥의 전화번호 오탐 확인 |
| `phone_positive` | 300 | 연락처, 전화번호, 회신 번호, 보호자 연락처 등 실제 전화번호 문맥 재현율 확인 |
| `hard_negative_email` | 300 | 예시, 샘플, 더미, 테스트, 형식, 매뉴얼, 문서, 목업 문맥의 이메일 오탐 확인 |
...
| 예시 문맥 확장 | `configs/context_rules.yaml` | `placeholder`, `fixture`, `목업`, `교육 자료`, `검증용`, `스토리북`, `seed`, `synthetic`, `데모`, `템플릿`, `가이드`, `샌드박스`, `형식`, `매뉴얼`, `문서용`을 example context로 본다. |
| contact 예시 문맥 정책 통과 | `src/pii_guardrail/policy.py` | 전화번호·이메일 후보가 예시 문맥에 있으면 후보는 남기되 최종 action은 `pass`로 둔다. |
...
2. 예시·샘플·템플릿 여부는 정규식이 아니라 context scorer와 policy router에서 처리한다.
