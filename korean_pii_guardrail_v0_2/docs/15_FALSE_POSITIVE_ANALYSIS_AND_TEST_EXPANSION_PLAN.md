...
현재 기준은 `reports/release_gate_v0_2.json`의 5,000개 synthetic release gate 결과다. 이 문서의 5장 이후에는 2026-05-22 당시의 개선 계획과 단계별 보정 기록이 포함되어 있으므로, 최신 전체 수치는 아래 표를 우선 기준으로 삼는다.
...
| actionable 정밀도 | 0.9974 |
| actionable 재현율 | 0.8776 |
| 고위험 개인정보 재현율 | 0.9647 |
| actionable 고위험 개인정보 재현율 | 0.9308 |
| 조사 경계 정확도 | 1.0000 |
| 잘못된 offset 수 | 0 |
| 원문 개인정보 로그 유출 수 | 0 |
| 감사 이벤트 원문 값 유출 수 | 0 |
...
최신 기준에서 구조형 PII 버킷은 전체 precision/recall/F1이 모두 1.0000이다. 따라서 이 문서의 초기 가설 중 법인등록번호/RRN, 계좌/사업자/전화/주문번호성 숫자 혼동은 stage별 보정이 완료된 historical item으로 보고, 현재 우선순위는 `PERSON_NAME` NER 후보 품질과 hard negative candidate noise로 둔다.
...
| actionable 정밀도 | 0.9974 | 0.9900 이상 유지 |
| actionable 재현율 | 0.8776 | 0.9000 이상 |
| 고위험 개인정보 재현율 | 0.9647 | 0.9600 이상 유지 |
