...
Latest result note: 2026-05-25 기준 `reports/release_gate_v0_2.json`의 5,000건 release gate는 pass 상태다. 본 문서의 데이터셋 구성은 초기 계획이며, 최신 실측 결과는 8.1절을 우선 참고한다.
...
| Metric | 설명 |
|---|---|
| Entity precision/recall/F1 | entity type 기준. candidate-level public span 전체를 prediction으로 본다. |
| Actionable entity precision/recall/F1 | `mask`, `hash`, `block`으로 최종 조치된 span만 prediction으로 본다. 실제 마스킹 품질 판단에는 이 값을 함께 본다. |
...
| Actionable high-risk recall | P0/P1 gold 중 최종 조치된 span 기준 재현율 |
...
### 8.1 2026-05-25 release gate result
...
- 구조형 PII release gate는 안정적이다.
- `CUSTOMER_ID`, `EMPLOYEE_ID`, `STUDENT_ID`는 production 기본 release gate에서 제외한다. 고객번호/사번/학번은 조직별 포맷과 validator/checksum 설정이 필요한 custom identifier profile 범위다.
- 남은 주요 failure는 `PERSON_NAME` FN, hard negative candidate FP, `ADDRESS_FULL` type confusion이다.
- hard negative FP는 대부분 최종 action이 `pass`이므로 candidate-level metric과 actionable metric을 분리해서 해석해야 한다.
- real NER가 포함된 latency는 deterministic-only latency와 분리 계측해야 한다.
...
확장 데이터셋은 entity 혼동을 단계별로 분리해 관리한다. `stage4_corporate_rrn_expanded`는 `CORPORATE_REG_NO`, 즉 법인등록번호와 `RRN`, 즉 주민등록번호의 유형 혼동을 검증하는 400건 데이터셋이다. `stage5_numeric_identifier_expanded`는 `BUSINESS_REG_NO`, 즉 사업자등록번호, `BANK_ACCOUNT`, 즉 계좌번호, `PHONE_MOBILE`/`PHONE_LANDLINE`, 즉 전화번호와 주문번호성 숫자 식별자 혼동을 검증하는 900건 데이터셋이다.

stage5의 주요 산출물은 `reports/eval_stage5_numeric_identifier_expanded.json`, `reports/failure_cases_stage5_numeric_identifier_expanded.jsonl`, `reports/audit_safety_stage5_numeric_identifier_expanded.json`이다.
...
python scripts/generate_stage4_corporate_rrn_markers.py

python scripts/build_marker_eval_dataset.py \
  --input data/eval/markers/stage4_corporate_rrn_expanded.jsonl \
  --output data/eval/generated/stage4_corporate_rrn_expanded.jsonl \
  --validate

python scripts/run_eval.py \
  --config-dir configs \
  --dataset data/eval/generated/stage4_corporate_rrn_expanded.jsonl \
  --output reports/eval_stage4_corporate_rrn_expanded.json \
  --failure-output reports/failure_cases_stage4_corporate_rrn_expanded.jsonl \
  --audit-safety-output reports/audit_safety_stage4_corporate_rrn_expanded.json \
  --mock-ner

python scripts/generate_stage5_numeric_identifier_markers.py

python scripts/build_marker_eval_dataset.py \
  --input data/eval/markers/stage5_numeric_identifier_expanded.jsonl \
  --output data/eval/generated/stage5_numeric_identifier_expanded.jsonl \
  --validate

python scripts/run_eval.py \
  --config-dir configs \
  --dataset data/eval/generated/stage5_numeric_identifier_expanded.jsonl \
  --output reports/eval_stage5_numeric_identifier_expanded.json \
  --failure-output reports/failure_cases_stage5_numeric_identifier_expanded.jsonl \
  --audit-safety-output reports/audit_safety_stage5_numeric_identifier_expanded.json \
  --mock-ner
```
