...
5. output target별 masking policy
...
| 입력 | 단일 문자열 `text` |
| 스캔 위치 | `input`, `output` |
| 전처리 | Unicode/NFKC, dash 정규화, zero-width 제거, 숫자 variant, raw offset map |
...
| 마스킹 | label mask, partial mask, pseudonym, HMAC hash, block |
...
| 복원형 토큰화 | v0.2 기본값은 비복원 label mask/pseudonym |
...
- `MockNERDetector`
- `FinetunedNERDetector` placeholder
- label mapping
- calibrated confidence 입력 계약
...
| NER 지연 | 모델 준비가 늦어짐 | mock NER로 pipeline 먼저 개발 |
