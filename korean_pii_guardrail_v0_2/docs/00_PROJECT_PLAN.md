...
- real NER activation은 pipeline/evaluation 안정화 이후 gated task로 분리했으며, 최신 상태는 아래 note를 따른다.

2026-05-24 상태: real NER v3는 `ner.finetuned.klue-roberta-large-v3`로 release gate에 연결되어 있으며, 최신 5,000건 release gate는 pass 상태다. 다만 NER score calibration, `PERSON_NAME` recall/precision, `ADDRESS` granularity, real NER latency 별도 계측은 후속 backlog로 남아 있다.
...
| NER 산출물 접근 불일치 | NER 명세의 local artifact 경로가 workspace에 없을 수 있음 | real NER release gate 경로는 연결됨. 단, artifact 재현성과 fallback mock/disabled path는 계속 유지 |
