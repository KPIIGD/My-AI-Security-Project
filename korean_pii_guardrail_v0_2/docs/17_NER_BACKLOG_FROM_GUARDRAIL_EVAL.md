...
- 퍼저 장기 과제: release gate보다 더 공격적인 변형에서는 IP/MAC, 사번/학번/환자번호, 차량번호, 기기 ID 같은 숫자형/라벨 기반 탐지의 normalization과 타입 충돌 개선이 필요하다. 이는 NER팀 책임이 아니라 regex/normalization/resolver 후속 과제다.
...
| actionable precision, 즉 실제 조치 대상 정밀도 | 0.9974 |
| actionable recall, 즉 실제 조치 대상 재현율 | 0.8776 |
| actionable F1 | 0.9337 |
| actionable high-risk recall, 즉 실제 조치 기준 고위험 재현율 | 0.9308 |
...
2026-05-24 기준 `reports/release_gate_v0_2.json`에서 `PERSON_NAME`은 전체 기준 precision 0.6755, recall 0.8697, F1 0.7604로 남은 주요 약점이다. actionable 기준에서는 guardrail 정책 덕분에 precision이 높지만, 후보 생성 관점에서는 이름을 놓치거나 hard negative에서 불필요한 이름 후보를 내는 양쪽 문제가 같이 있다.
