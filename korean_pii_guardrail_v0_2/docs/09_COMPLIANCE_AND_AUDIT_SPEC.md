...
v0.2 guardrail은 단순 탐지기가 아니라 masked/pseudonymized output을 생성하는 시스템이다. 따라서 다음을 분리해 관리한다.
...
| 가명처리 | masker/pseudonymizer | masked text, public spans |
...
## 9. Pseudonymization result report
...
가명처리 후 특정 개인 식별 가능성이 발견되면 다음 순서로 처리한다.
...
| Pseudonymization Plan | Pipeline Owner |
| Pseudonymization Result Report | Evaluation Owner |
