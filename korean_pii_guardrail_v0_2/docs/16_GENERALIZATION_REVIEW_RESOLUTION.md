...
| 검증 | 결과 | 근거 |
|---|---:|---|
| 집중 pytest | 71 passed | context/policy/pipeline 리뷰 회귀 테스트 |
| 전체 pytest | 1233 passed | `origin/main` 병합 뒤 전체 단위 테스트 |
...
| release gate actionable | pass | `actionable_precision=0.9974`, `actionable_recall=0.8776`, `actionable_f1=0.9337` |
| release gate 핵심 recall | pass | structured PII bucket F1 1.0, phone/email recall 1.0 |
| release gate 조사 경계 | pass | boundary accuracy 1.0 |
...
최신 release gate 기준 잔여 failure row는 `PERSON_NAME` FN 156건, hard negative candidate FP 750건, `ADDRESS_FULL` type confusion 13건이다. 구조형 PII 경로는 release gate 범위에서 안정적이며, 잔여 과제는 `docs/17_NER_BACKLOG_FROM_GUARDRAIL_EVAL.md`의 NER backlog와 `reports/release_gate_failure_analysis_ko.md`의 최신 분석으로 분리한다.
